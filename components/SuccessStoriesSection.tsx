    ...
    <section id="success-stories" className="py-32 bg-ikigai-bg scroll-mt-20" aria-labelledby="success-stories-title">
      <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8, ease: "easeOut" }} className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center mb-24">
          ...
          <p className="text-ikigai-muted font-light mt-6 max-w-2xl mx-auto">Witness the transformation of our students as they overcome obstacles and reach for their dreams.</p>
        </div>
          ...
          renderItem={(story: any) => (
            <div className="group h-full flex flex-col">
              <div className="relative aspect-square w-full rounded-square-image overflow-hidden mb-8 shadow-lg group-hover:shadow-2xl transition-all duration-700 ease-out">
                <motion.div initial={{ scale: 1.1 }} whileInView={{ scale: 1 }} transition={{ duration: 1.2, ease: [0.16, 1, 0.3, 1] }} className="absolute inset-0">
                  <Image src={story.image} alt={`Success story: ${story.name}`} fill className="object-cover group-hover:scale-105 group-hover:brightness-[0.85] transition-all duration-1000 ease-out" referrerPolicy="no-referrer" />
                </motion.div>
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-ikigai-dark/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end p-8">
                  <p className="text-white text-sm font-light leading-relaxed transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">{story.description}</p>
                </div>
              </div>
